...
#######
my $chaintype="MCMC";
my $chainlength="16777216";
my $logperiod="2048";##For all written logs
my $logscreenperiod="2048";##STDOUT
my $storeperiod="2048";##Backup state to resume afterwards if necessary
my $empiricalFreqs=0;
my $relaxed_STree=0;
my $log_genes=0;
my $ploidy=1;
...
{
	print($XML '<?xml version="1.0" encoding="UTF-8" standalone="no"?><beast beautitemplate=\'StarBeast2UCLN\' beautistatus=\'noAutoSetClockRate\' namespace="beast.core:beast.evolution.alignment:beast.evolution.tree.coalescent:beast.core.util:beast.evolution.nuc:beast.evolution.operators:beast.evolution.sitemodel:beast.evolution.substitutionmodel:beast.evolution.likelihood" required="starbeast2 v0.13.12" version="2.4">');
}
...
print($XML "<run id=\"mcmc\" spec=\"$chaintype\" chainLength=\"$chainlength\" storeEvery=\"$storeperiod\">\n");
	...
	print($XML "\t<state id=\"state\" storeEvery=\"$storeperiod\">\n");##All components of the chain state (parameters and trees)
		...
		if($relaxed_STree == 1)
		{
			print($XML "\t\t<stateNode id=\"branchRates.Species\" spec=\"parameter.IntegerParameter\" dimension=\"0\" lower=\"0\">1</stateNode>\n");
			print($XML "\t\t<parameter id=\"branchRatesStdev.Species\" lower=\"0.01\" name=\"stateNode\">0.3</parameter>\n");
		}
	...
	print($XML "\t<distribution id=\"posterior\" spec=\"util.CompoundDistribution\">\n");
		...
		print($XML "\t\t<distribution id=\"prior\" spec=\"util.CompoundDistribution\">\n");
			...
			##I don't need to push one gamma per each, I can reuse them I should implement this
			#Gene-tree related
			foreach my $locus (@loci)
			{
				#SiteHeter
		...
		for (my $locus_i=0; $locus_i<scalar @loci; ++$locus_i)
		{
			my $locus=$loci[$locus_i];
			...
			print($XML "\t\t\t<distribution id=\"treeLikelihood.$locus\" spec=\"TreeLikelihood\" data=\"\@$locus\" tree=\"\@Tree.t:$locus\">\n");
				...
				print($XML "\t\t\t\t<siteModel id=\"SiteModel.s:$locus\" spec=\"SiteModel\" gammaCategoryCount=\"4\" shape=\"\@gammaShape.s:$locus\">\n");
					...
					print($XML "\t\t\t\t\t<substModel id=\"gtr.s:$locus\" spec=\"GTR\" rateAC=\"\@rateAC.s:$locus\" rateAG=\"\@rateAG.s:$locus\" rateAT=\"\@rateAT.s:$locus\" rateCG=\"\@rateCG.s:$locus\" rateGT=\"\@rateGT.s:$locus\">\n"); #Substitution model
						...
						{
							print($XML "\t\t\t\t\t\t<frequencies id=\"empiricalFreqs.s:$locus\" spec=\"Frequencies\" data=\"\@$locus\"/>\n");
						}
	...
	print($XML "\t<operator id=\"Reheight.t:Species\" spec=\"starbeast2.NodeReheight2\" taxonset=\"\@taxonsuperset\" tree=\"\@Tree.t:Species\" weight=\"75.0\">\n");
	foreach my $locus (@loci)
	{
	...
	print($XML "\t<operator id=\"coordinatedUniform.t:Species\" spec=\"starbeast2.CoordinatedUniform\" speciesTree=\"\@Tree.t:Species\" weight=\"15.0\">\n");
	foreach my $locus (@loci)
	{
		print($XML "\t\t<geneTree idref=\"Tree.t:$locus\"/>\n");
	}
	...
	print($XML "\t<operator id=\"coordinatedExponential.t:Species\" spec=\"starbeast2.CoordinatedExponential\" speciesTree=\"\@Tree.t:Species\" weight=\"15.0\">\n");
	foreach my $locus (@loci)
	{
		print($XML "\t\t<geneTree idref=\"Tree.t:$locus\"/>\n");
	}
	...
	print($XML "\t<operator id=\"TreeScaler.t:Species\" spec=\"ScaleOperator\" scaleFactor=\"0.95\" tree=\"\@Tree.t:Species\" weight=\"3.0\"/>\n");
	print($XML "\t<operator id=\"TreeRootScaler.t:Species\" spec=\"ScaleOperator\" rootOnly=\"true\" scaleFactor=\"0.7\" tree=\"\@Tree.t:Species\" weight=\"3.0\"/>\n");
	print($XML "\t<operator id=\"UniformOperator.t:Species\" spec=\"Uniform\" tree=\"\@Tree.t:Species\" weight=\"15.0\"/>\n");
	print($XML "\t<operator id=\"SubtreeSlide.t:Species\" spec=\"SubtreeSlide\" size=\"0.002\" tree=\"\@Tree.t:Species\" weight=\"15.0\"/>\n");
	print($XML "\t<operator id=\"Narrow.t:Species\" spec=\"Exchange\" tree=\"\@Tree.t:Species\" weight=\"15.0\"/>\n");
	print($XML "\t<operator id=\"Wide.t:Species\" spec=\"Exchange\" isNarrow=\"false\" tree=\"\@Tree.t:Species\" weight=\"15.0\"/>\n");
	print($XML "\t<operator id=\"WilsonBalding.t:Species\" spec=\"WilsonBalding\" tree=\"\@Tree.t:Species\" weight=\"15.0\"/>\n");
	print($XML "\t<operator id=\"speciationRateScale.t:Species\" spec=\"ScaleOperator\" parameter=\"\@speciationRate.t:Species\" scaleFactor=\"0.5\" weight=\"1.0\"/>\n");
	print($XML "\t<operator id=\"popMeanScale.Species\" spec=\"ScaleOperator\" parameter=\"\@popMean.Species\" scaleFactor=\"0.75\" weight=\"1.0\"/>\n");
	if($relaxed_STree == 1)
	{
		print($XML "\t<operator id=\"branchRatesCycle.Species\" spec=\"starbeast2.DiscreteRateCycle\" k=\"2\" optimise=\"false\" treeRates=\"\@branchRates.Species\" weight=\"9.0\"/>\n");
	...
	print($XML "\t<logger id=\"tracelog\" fileName=\"$outname.log\" logEvery=\"$logperiod\" model=\"\@posterior\" sort=\"smart\">\n");
		...
		if($relaxed_STree==1)
		{
			print($XML "\t\t<log idref=\"branchRatesStdev.Species\"/>");
		}
		if($log_genes==1)
		{
			for my $locus (@loci)
	...
	#Species tree logger
	print($XML "\t<logger id=\"speciesTreeLogger\" fileName=\"${outname}.trees\" logEvery=\"$logperiod\" mode=\"tree\">\n");
	if($relaxed_STree==0)
	{
	...
	{
		print($XML "\t\t<log id=\"SpeciesTreeLoggerX\" spec=\"starbeast2.SpeciesTreeLogger\" speciesTree=\"\@Tree.t:Species\" branchratemodel=\"\@branchRatesModel.Species\"/>\n");
	}
    	print($XML "\t</logger>\n");
	...
	if($log_genes==1)
	{
		...
		for my $locus (@loci)
		{
			print($XML "\t<logger id=\"treelog.t:$locus\" fileName=\"${outname}_\$(tree).trees\" logEvery=\"$logperiod\" mode=\"tree\">\n");
	        		print($XML "\t\t<log id=\"TreeWithMetaDataLogger.t:$locus\" spec=\"beast.evolution.tree.TreeWithMetaDataLogger\" tree=\"\@Tree.t:$locus\"/>\n");
			print($XML "\t</logger>\n");
		}
	}
	print($XML "\t<operatorschedule id=\"operatorSchedule\" spec=\"OperatorSchedule\">\n");
		print($XML "\t\t<subschedule id=\"operatorSubschedule\" spec=\"OperatorSchedule\" operatorPattern=\"^.*Species\$\" weight=\"20.0\" weightIsPercentage=\"true\"/>");
	print($XML "\t</operatorschedule>\n");
...
sub pushNormal
{
	die "This push function has not been implemented yet";
}
